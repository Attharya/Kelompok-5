...
- **Setoran (Deposit):** Numeric value entered when Option `2` is chosen, added to the balance.
- **Penarikan (Withdrawal):** Numeric value entered when Option `3` is chosen, deducted from the balance.
...
> **Note:** the program does not validate these inputs. A deposit or withdrawal of `0`, a negative number, or non-numeric text is accepted the same way as any other value — see Option 2 and Option 3 below.
...
- **Option 2 (Setor Uang)** → the program asks for the deposit amount and always adds it to the balance, with no validation:
  ...
  - There is currently no check for `jumlah <= 0` or non-numeric input; a deposit of `0`, a negative amount, or `NaN` (from non-numeric text) is still treated as a successful transaction, and a `NaN` deposit permanently corrupts the balance for the rest of the session.
- **Option 3 (Tarik Uang)** → the program asks for the withdrawal amount and checks it against the balance:
  ...
  - There is no separate check for `jumlah <= 0`. A negative withdrawal amount still satisfies `jumlah <= saldo`, so it is treated as a valid withdrawal and, because subtracting a negative number is addition, it *increases* the balance instead of decreasing it.
